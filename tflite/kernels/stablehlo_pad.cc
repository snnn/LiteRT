...
/// Recursively copies a tensor region with independent input and output strides.
void StridedCopy(const char* input, absl::Span<const int64_t> input_shape,
                 ...
                 int64_t element_size, int depth) {
 ...
 public:
  ...
  TfLiteStatus Setup(TfLiteContext* context, absl::Span<const int> dims,
    ...
    for (int i = 0; i < rank; ++i) {
      TF_LITE_ENSURE_MSG(
          context,
          output_shape_[i] <= std::numeric_limits<int>::max(),
    ...
    TF_LITE_ENSURE_OK(
        context,
        CheckedShapeProduct(context, absl::MakeConstSpan(input_dims, rank),
                            "StableHLO Pad input size overflowed.",
                            input_elements));
    size_t input_bytes = 0;
    TF_LITE_ENSURE_MSG(
        context,
        MultiplyAndCheckOverflow(input_elements,
    ...
    TF_LITE_ENSURE_OK(
        context,
        CheckedShapeProduct(context, absl::MakeConstSpan(output_dims, rank),
                            "StableHLO Pad output size overflowed.",
                            output_elements));
    size_t output_bytes = 0;
    TF_LITE_ENSURE_MSG(
        context,
        MultiplyAndCheckOverflow(output_elements,
                                 static_cast<size_t>(element_size),
    ...
    for (int i = 0; i < rank; ++i) {
      output_offset += CheckedInt<int64_t>(std::max<int64_t>(
                           edge_pad_low_[i], 0)) *
    ...
    auto DivNegRoundAwayOrZero =
        [](int64_t num, int64_t denum, int64_t* result) -> TfLiteStatus {
      if (denum <= 0) {
        return kTfLiteError;
      }
      if (num >= 0) {
    ...
    for (int i = 0; i < rank; ++i) {
      ...
      TF_LITE_ENSURE_MSG(context,
                         DivNegRoundAwayOrZero(edge_pad_low_[i],
                                               interior_step_[i],
                                               &low_crop) == kTfLiteOk,
                         "StableHLO Pad crop computation overflowed.");
    ...
    output_offset_ = output_offset.Value();
    TF_LITE_ENSURE_MSG(context, input_offset_ >= 0 &&
                                    static_cast<size_t>(input_offset_) <
                                        input_bytes,
                       "StableHLO Pad input offset is out of bounds.");
    TF_LITE_ENSURE_MSG(context, output_offset_ >= 0 &&
  ...
  void Apply(const char* input, const char* padding_value, char* output) const {
    ...
    FillBuffer(absl::MakeSpan(output, output_size_),
               absl::MakeConstSpan(padding_value,
                                   static_cast<size_t>(element_size_)));
    if (output_size_ == 0 || !has_input_copy_) {
      return;
    }
    const int rank = static_cast<int>(rank_);
    StridedCopy(input + input_offset_,
                absl::MakeConstSpan(input_shape_, rank),
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_OK(context,
                    GetInputSafe(context, node, PadData::kInput,
                                 &input_tensor));
  const TfLiteTensor* padding_value_tensor;
  TF_LITE_ENSURE_OK(context,
                    GetInputSafe(context, node, PadData::kPaddingValue,
  ...
  TF_LITE_ENSURE_OK(context,
                    GetOutputSafe(context, node, PadData::kOutput,
                                  &output_tensor));
