...
#include "tflite/kernels/internal/reference/sparse_ops/fully_connected.h"
...
inline TfLiteStatus CheckTypes(TfLiteContext* context,
  ...
  if (is_quantized) {
    ...
    } else if (filter->type == kTfLiteInt16) {
    ...
    switch (output->type) {
      ...
      case kTfLiteInt16:
        if (input->type == kTfLiteInt16) {
          if (filter->type == kTfLiteInt16) {
            if (filter->sparsity != nullptr) {
              TF_LITE_KERNEL_LOG(
                  context,
                  "Sparse int16 fully-connected weights are not supported.");
              return kTfLiteError;
            }
            const bool use_int64_bias =
                data->quantized_bias_type == kTfLiteInt64;
            if (use_int64_bias) {
              is_per_channel ? FullyConnectedPerChannelInt16FilterInt16<int64_t>(
                                   data, input, filter, bias, output)
                             : FullyConnectedInt16FilterInt16<int64_t>(
                                   data, input, filter, bias, output);
            } else {
              is_per_channel ? FullyConnectedPerChannelInt16FilterInt16<int32_t>(
                                   data, input, filter, bias, output)
