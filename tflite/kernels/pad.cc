...
TfLiteStatus ValidatePaddingsShape(TfLiteContext* context,
  ...
  TF_LITE_ENSURE_EQ(context, op_context.dims,
                    NumDimensions(op_context.input));
  TF_LITE_ENSURE_OK(
      context, ValidatePaddingsMatrixShape(context, op_context.paddings));
...
TfLiteStatus ValidateConstantValuesShape(TfLiteContext* context,
  ...
  TF_LITE_ENSURE_OK(
      context, ValidateTensorShape(context, op_context.constant_values));
...
TfLiteStatus ValidateTensorData(TfLiteContext* context,
  ...
  size_t count = 0;
  TF_LITE_ENSURE_MSG(context,
                     CheckedNumElements(tensor, count) == kTfLiteOk,
...
TfLiteStatus ResizeOutputTensor(TfLiteContext* context,
  ...
  for (int idx = 0; idx < op_context.dims; ++idx) {
    TF_LITE_ENSURE_OK(
        context, GetPaddedOutputDimension(context, input_shape.Dims(idx),
  ...
  if (op_context.resizing_category == ResizingCategory::kImageStyle) {
    ...
    TF_LITE_ENSURE_MSG(
        context,
        CheckedNumElements(output_size.get(), output_num_elements_int) ==
            kTfLiteOk,
...
template <typename PaddingIntegerType>
TfLiteStatus GetPadParams(TfLiteContext* context,
                          const PadContext& op_context,
  ...
  *op_params = tflite::PadParams();
  ...
  int paddings_total = 0;
  TF_LITE_ENSURE_MSG(
      context, CheckedNumElements(op_context.paddings, paddings_total) ==
                   kTfLiteOk,
      "Pad paddings size overflowed.");
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_OK(context,
                    ValidateConstantValuesShape(context, op_context));
  ...
  tflite::PadParams op_params;
  TF_LITE_ENSURE_OK(context,
                    GetPadParams(context, op_context, &op_params));
