 ...
 public:
  ...
  void PerChannelSymmetricQuantizeAndPopulate(
    ...
    if (t->type == kTfLiteInt16) {
      ...
      int adjusted_channel_index =
          channel_index + kPerChannelMaxDim -
      ...
      for (indices[0] = 0; indices[0] < tensor_dims.Dims(0); indices[0]++) {
        for (indices[1] = 0; indices[1] < tensor_dims.Dims(1); indices[1]++) {
          for (indices[2] = 0; indices[2] < tensor_dims.Dims(2);
               indices[2]++) {
            for (indices[3] = 0; indices[3] < tensor_dims.Dims(3);
              ...
              const int32_t quantized_value = static_cast<int32_t>(
                  std::round(input_data[tensor_index] * scales_inv[channel_idx]));
              quantized_output[tensor_index] = static_cast<int16_t>(
                  std::min<int32_t>(std::numeric_limits<int16_t>::max(),
                                    std::max<int32_t>(
