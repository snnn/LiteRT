...
TfLiteStatus GetPaddedOutputShape(
  ...
  for (size_t i = 0; i < padding_matrix->num_dims; ++i) {
    ...
    TF_LITE_ENSURE_MSG(
        ...
        "MirrorPad paddings must be no greater than the dimension size.");
  ...
  TF_LITE_ENSURE_MSG(context,
                     CheckedNumElements(shape->get(), output_num_elements) ==
                         kTfLiteOk,
...
int GetFlatIndex(int index, EvalData<T>* eval_data) {
  ...
  for (size_t i = 0; i < eval_data->num_dims; ++i) {
    ...
    index_in_input = GetInputDimension(
        dimension_index, left_pad, right_pad, eval_data->input_dims->data[i],
        eval_data->offset);
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_OK(context,
                    ValidatePaddingMatrix(context, input_tensor,
                                          padding_matrix));
  PaddingMatrixData padding_data;
  TF_LITE_ENSURE_OK(context,
                    GetPaddingMatrixData(context, padding_matrix,
                                         &padding_data));
  ...
  if (IsDynamicTensor(output_tensor)) {
    TF_LITE_ENSURE_STATUS(
        context->ResizeTensor(context, output_tensor, expected_shape.release()));
  } else {
  ...
  for (int i = input_dims - 2; i >= 0; i--) {
    TF_LITE_ENSURE_MSG(
        context, output_shape.CheckedSizeFromDimension(
                     /*start=*/i + 1, output_dims_num_elements[i]),
        "MirrorPad output stride overflowed.");
    TF_LITE_ENSURE_MSG(
        context, input_shape.CheckedSizeFromDimension(
  ...
  int output_size = 0;
  TF_LITE_ENSURE_MSG(context,
                     CheckedNumElements(output_tensor, output_size) ==
                         kTfLiteOk,
                     "MirrorPad output size overflowed.");
