...
absl::Status FallbackBF16WeightsToFp32(
  ...
  for (auto& [name, tensor] : weights_handle) {
    // The per-layer embedding table converts only the rows requested by the
    // CPU lookup; leave its mapped BF16 storage intact.
    if (tensor.GetType() != Type::kBF16 ||
        absl::EndsWith(name, "embed_tokens_per_layer.weight")) {
      continue;
    }
    LRT_TENSOR_ASSIGN_OR_RETURN(Buffer & buffer, tensor.GetBuffer());
...
absl::Status Run(const std::string& weights_path,
  ...
  if (tokenizer.has_value() && model_variant == ModelVariant::kE4B) {
    ...
    if (!absl::StrContains(raw_prompt, start_of_turn)) {
