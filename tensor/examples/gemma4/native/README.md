...
bundle; it is not a general E4B or arbitrary checkpoint loader. The separate
...
| [decode_trace.h](decode_trace.h) | Mark measured forced-decode intervals for an external memory-controller collector. |
...
For the cross-runtime design and measurement context, see the
[KV cache layout reference](../../../docs/kv_cache_layout.md). It distinguishes
this active INT8 bank from the older transposed-V reference path and the
standard safetensors example.
...
of stage calls and is narrower than elapsed time. Memory snapshots, stage
dumps and warmups must not be mixed into ordinary timing results.

For hardware read/write traffic, the optional `LITERT_NATIVE_TRACE_FIFO`
channel marks only forced decode calls in measured sessions. The
[decode memory-traffic guide](../../../docs/native_decode_memory_traffic.md)
shows how to use the shared LiteRT-LM IMC collector and explains counter scope,
token accounting, and Android limitations. This measures bytes moved through
the monitored memory controllers; `--memory_report` records memory footprint
and must be disabled during a bandwidth capture.
...
matching argmax IDs is weaker evidence. The copied source and fixture hashes
are recorded in [migration_provenance.json](migration_provenance.json); results
from the original executable do not certify a newly rebuilt port.
