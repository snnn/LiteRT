...
absl::Status WriteLiveRun(const BenchCase& item,int run_index,bool warmup,int threads,bool reused,
    ...
    bool trace_enabled,double trace_elapsed_ms) {
  ...
  out<<"{\"schema_version\":1,\"runner\":\"xnnpack_tensor_live_int8\",\"case_id\":\""<<item.id
     ...
     <<",\"decode_trace_enabled\":"<<(trace_enabled?"true":"false")
     <<",\"decode_trace_section\":"<<(trace_enabled?"\"decode\"":"null")
     <<",\"measured_decode_calls\":"<<(trace_enabled&&!warmup?item.forced.size():0)
     <<",\"decode_trace_elapsed_ms\":"<<trace_elapsed_ms
...
absl::Status RunLiveCase(LiveRuntime& rt,const Config& config,const LoadedTensors& loaded,
    ...
    bool reused,bool compiled,bool dump,double load_ms,const std::string& dir,DecodeTrace& trace) {
  ...
  passes.reserve(item.forced.size()+1);
  DecodeTraceSection decode_section(trace,!warmup);
  for(size_t i=0;i<=item.forced.size();++i) {
    if(i==1)LRT_TENSOR_RETURN_IF_ERROR(decode_section.Begin());
    ...
    const double elapsed_ms=Milliseconds(start);
    if(i>0&&i==item.forced.size())LRT_TENSOR_RETURN_IF_ERROR(decode_section.End());
    PassResult pass{int(i),i?token:-1,bank.length(),pred,elapsed_ms,times,""};
    if(dump) {
  ...
  return WriteLiveRun(item,index,warmup,threads,reused,compiled,rt,load_ms,passes,dir,trace.enabled(),decode_section.elapsed_ms());
}
absl::Status LiveMain() {
  ...
  DecodeTrace trace;
  LRT_TENSOR_RETURN_IF_ERROR(trace.InitializeFromEnvironment());
  if(trace.enabled()) {
    if(absl::GetFlag(FLAGS_dump_full_logits)||absl::GetFlag(FLAGS_dump_cache)||
       absl::GetFlag(FLAGS_memory_report)||absl::GetFlag(FLAGS_trace_position)>=0)
      return absl::InvalidArgumentError("Decode tracing requires dumps, memory_report, and trace_position diagnostics to be disabled");
    for(const auto& c:cases)if(c.forced.empty())
      return absl::InvalidArgumentError("Decode tracing requires forced decode inputs in every case");
  }
  ...
  for(const auto& c:cases)for(int i=0;i<warmups+runs;++i) {
    ...
    LRT_TENSOR_RETURN_IF_ERROR(RunLiveCase(*rt,config,loaded,specs,c,index,warmup,threads,reuse,compiled,
      absl::GetFlag(FLAGS_dump_full_logits)&&!warmup&&index==0,load_ms,dir,trace));
