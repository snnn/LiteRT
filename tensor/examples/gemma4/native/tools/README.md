...
These tools inspect/export the published E2B `.litertlm` bundle, regenerate
fixed histories, and compare saved outputs. They perform no model inference,
builds or device actions. Large model files and captures remain outside the
source tree.
...
even when the chosen token is unchanged. The 18 local comparator tests cover
