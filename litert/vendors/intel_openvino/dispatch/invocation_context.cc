...
LiteRtDispatchInvocationContextT::Create(
  ...
  // select THIS partition's subgraph (by function_name, which the plugin sets
  // to the graph name; fall back to the sole subgraph). The subgraph's weights
  // are resolved against the pool at import below:
  ...
  // |global_graph| is kept at function scope (its spans alias
  // exec_bytecode_buffer, which outlives this function) so the GPU bank fill
  // below can reuse it -- no second parse.
  ...
  if (npu_shared) {
    ...
    const uint8_t* buf_end =
        buf_base + exec_bytecode_buffer->offset + exec_bytecode_buffer->size;
  ...
  if (gpu_shared) {
    // Reuse the graph parsed above (its buffer spans alias
    // exec_bytecode_buffer, which outlives this function) -- no second parse,
    // no pool copy. The GPU bank resolves each const_map buffer_id to its pool
    // bytes via the graph's ascending-id BufferEntry list.
